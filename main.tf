...
resource "aws_eip" "eip" {
  instance = aws_instance.fidrox-instance.id
  domain   = "vpc"
}

resource "aws_ebs_volume" "fidrox-volume" {
  availability_zone = aws_instance.fidrox-instance.availability_zone
  size              = 40
...
resource "aws_volume_attachment" "example_attachment" {
  ...
  volume_id   = aws_ebs_volume.fidrox-volume.id
  instance_id = aws_instance.fidrox-instance.id

  # Forces detachment of the volume before destroying, allowing Terraform to manage the volume's lifecycle
  force_detach = true
}

resource "aws_instance" "fidrox-instance" {
